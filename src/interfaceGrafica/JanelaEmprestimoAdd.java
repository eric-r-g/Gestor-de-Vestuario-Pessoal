...
public class JanelaEmprestimoAdd extends ModeloPagina{
	...
	protected void atualizarFormulario() {
		...
		painelFormulario.add(new JLabel("Item para emprestimo: "));
		painelFormulario.add(dropdownItem);
	...
	protected void configurarContentPane() {
		...
		botaoSubmeter.addMouseListener(new MouseAdapter() {
        	public void mouseClicked(MouseEvent e) {
        		try {
        			UUID id_item = ((ComboItem) dropdownItem.getSelectedItem()).getId();
        			IEmprestavel item = null;
        			if (id_item == null) {
        				throw new EntradaInvalidaException("Nenhum item selecionado");
        			} else {
        				item = (IEmprestavel) itemManager.retornarItemPorId(id_item);
        			}
        			String nome = formNome.getText();
        			int dia = Integer.valueOf(formDia.getText());
        			System.out.print(dia);
        			int ano = Integer.valueOf(formAno.getText());
        			Month mes = ((Month) dropdownMes.getSelectedItem());
        			
        			emprestimoManager.novoEmprestimo(item, nome, ano, mes, dia);
        			emprestimoManager.atualizarBD();
