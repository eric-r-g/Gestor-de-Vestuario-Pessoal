...
public class JanelaItensAdd extends ModeloPagina{
	...
	public void setItemEditar(UUID itemId) {
        this.itemEditar = itemId;
        try {
            modoEdicao();
	...
	protected void modoAdicao() {
		...
		formLojaOrigem.setText("");
		formModelo.setText("");
		formPreco.setText("");
		formMaterial.setText("");
		formNumeracao.setText("");
		formTamanho.setText("");
		itemEditar = null;
	...
	protected void modoEdicao() throws ArrayVaziaException, ElementoInexistenteException {
		...
		dropdownTipo.setSelectedItem(tipoItem.fromString(item.getTipo()));
		...
		switch(item.getTipo()) {
			...
			case "superior" -> {
				...
				boolean isExterno = ((PecaSuperior) item).isExterno();
	            
	            if (formExterno != null) {
	                formExterno.clearSelection();
	                if (isExterno) {
			...
			case "unica" -> formTamanho.setText(Integer.toString(((PecaUnica) item).getTamanho()));
			case "intima" -> formTamanho.setText(Integer.toString(((RoupaIntima) item).getTamanho()));
		}
	...
	protected void configurarContentPane() {
		...
		botaoSubmeter = new JButton("adicionar");
        		...
        		try {
	                ...
	                try {
	                	if(itemEditar == null) {
	                		...
	                		Item antigo = itemManager.retornarItemPorId(itemEditar);
	                		itemManager.atualizarItem(tipo, modelo, lojaOrigem, conservacao, preco, extras, antigo);
	                	}
	                } catch (Exception erro) {
