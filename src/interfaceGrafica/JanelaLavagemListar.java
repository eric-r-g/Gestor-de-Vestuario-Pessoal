...
public class JanelaLavagemListar extends ModeloJanelaListar <ILavavel>{
	...
	protected void configuracoesExtrasContentPanel() {
		...
		JButton botaoLavar = new JButton("Realizar lavagem");
		botaoLavar.setMaximumSize(new Dimension(150, 30));
		
		botaoLavar.addMouseListener(new MouseAdapter() {
        	public void mouseClicked(MouseEvent e) {
        		try {
        			LocalDate data = LocalDate.now();
        			lavagemManager.realizarLavagem(data.getYear(), data.getMonth(), data.getDayOfMonth());
        		} catch(Exception erro) {
        			System.out.print(erro);
        		}
        	}
