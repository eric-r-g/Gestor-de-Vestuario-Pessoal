...
public class BDManager {
	...
	public void inserirLook(Look look) throws SQLException {
		try {
			String sql = "INSERT OR IGNORE INTO looks (id, id_superior, id_inferior, id_unica, id_calcado, id_acessorio) VALUES (?, ?, ?, ?, ?, ?)";
			PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	public void removerEmprestavel(IEmprestavel item) throws SQLException, ElementoInexistenteException {
	...
	public void atualizarEmprestavel(IEmprestavel item) throws SQLException {
		String sql = "UPDATE emprestimos SET data = ?, nome = ? WHERE id = ?";

		try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
			
			pstmt.setString(1, item.dataDeEmprestimo().toString());
			pstmt.setString(2, item.getNomeEmprestimo());
			pstmt.setString(3, item.getId().toString());
